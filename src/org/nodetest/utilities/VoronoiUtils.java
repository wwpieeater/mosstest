...
public class VoronoiUtils {
	...
	static VorParams vorToPoints(List<GraphEdge> listIn, double minX,
		...
		TreeMap<VorPoint, VorPoint> allPoints = new TreeMap<>(
				new Comparator<VorPoint>() {
					...
					public int compare(VorPoint o1, VorPoint o2) {
						if(o1.x-o2.x>4.0) return 1;
						else if(o1.x-o2.x<-4.0) return -1;
						else{
							if(o1.y-o2.y>4.0) return 1;
							else if(o1.y-o2.y<-4.0) return -1;
							else return 0;
						}
...
System.out.println("ListLinePlot[{");
		for (GraphEdge ge : listIn) {
			...
			if(allPoints.containsKey(p1)) {p1=allPoints.get(p1);points--;}
			if(allPoints.containsKey(p2)) {p1=allPoints.get(p2); points--;}
			...
			if (Math.abs(p1.y - minY) < 2)
				btmEdge.add(p1);
			if (Math.abs(p2.x - maxX) < 1)
				rightEdge.add(p2);
			if (Math.abs(p2.x - minX) < 1)
				leftEdge.add(p2);
			if (Math.abs(p2.y - maxY) < 1)
				topEdge.add(p2);
			if (Math.abs(p2.y - minY) < 1)
				btmEdge.add(p2);
			p1.register(p2);
			p2.register(p1);
			conns+=2;
		...
		//try {
			...
			while (topEdgeIterator.hasNext()) {
				VorPoint currentPoint = (VorPoint) topEdgeIterator.next();
		...
		//} catch (Exception e) {
		//	tmp = rightEdgeIterator.next();
		//	startPoint = tmp;
		//}
		//try {
			while (rightEdgeIterator.hasNext()) {
				VorPoint currentPoint = (VorPoint) rightEdgeIterator.next();
				currentPoint.register(tmp);
				tmp.register(currentPoint);
				conns++;
				tmp = currentPoint;
		...
		//try {
			while (btmEdgeIterator.hasNext()) {
				VorPoint currentPoint = (VorPoint) btmEdgeIterator.next();
				currentPoint.register(tmp);
				tmp.register(currentPoint);
				tmp = currentPoint;
			}
		...
		while (leftEdgeIterator.hasNext()) {
			VorPoint currentPoint = (VorPoint) leftEdgeIterator.next();
			currentPoint.register(tmp);
			tmp.register(currentPoint);
			conns++;
			tmp = currentPoint;
		...
		FindPolysLoop:while(true){
			...
			if(!currentPoint.directionsTaken[currentPoint.outputs[currentPoint.findInput(temp)][headingLeft?0:1]][headingLeft?0:1])
