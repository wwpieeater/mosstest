...
public class MossScriptEnv {
	public interface MossEventHandler {
		void processEvent(Entity actor, long posx, long posy, long posz,
				...
				MossFormspec fspec, MossInventoryAction action,
				String initiatingMessage) throws NullPointerException;
	}
	/*
	 * IMPORTANT, IMPORTANT, IMPORTANT. THIS CLASS IS THE ONLY CLASS THAT SCRIPTS CAN ACCESS.
	 * MAKE ALL FIELDS AND METHODS PRIVATE UNLESS IT IS INTENDED TO FACE UNTRUSTED SCRIPTS.
	 */
	...
	static void registerOnInvaction(MossEventHandler r){
		registeredScriptEvents.get(MossEvent.EvtType.EVT_FSPEC_INVACTION).add(r);
	}
	...
	static ArrayList<MossEventHandler> getHandlers(MossEvent.EvtType type, ScriptSandboxBorderToken tok) throws SecurityException{
		if(!(tok instanceof ScriptSandboxBorderToken)) throw new SecurityException("Attempt to access controlled resources in the script DMZ");
